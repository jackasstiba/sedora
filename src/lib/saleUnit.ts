...
/**
 * その価格が「**1個の値段**」とは言えない行（まとめ売り）か。
 *
 * なぜ要るか（2026-08-22 実測・公式ページから価格を取り始めた回）: 公式コレクションには
 * まとめ売りが混ざる。種別ごとに価格帯へ丸めると、
 *   『モノノ怪』**アクスタ付き3種セット** ¥22,880 → 「アクスタ ¥22,880」
 *   『モノノ怪』アクセサリー**3種セット**(アクリルスタンド付き) ¥21,340 → 「アクリルスタンド ¥21,340」
 * のように、**1個の値段のような顔で数万円**が出る。1個¥1,100の商品と同じ欄に並ぶので、
 * 読む側は見分けられない。
 *
 * 判定は2つ: ①既存の単位表記（【BOX】/カートン/パック入） ②「(◯種)セット」。
 * ②が要るのは、上の実例に BOX 表記が**無い**ため（①だけでは素通りする）。
 */
export function isBundlePriced(name: string): boolean {
  if (saleUnitLabel(name)) return true;
