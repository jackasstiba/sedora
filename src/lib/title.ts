...
export function hasProductSegment(source: string, title: string): boolean {
  ...
  return block !== null && !NON_PRODUCT_POST.test(block) && !CAMPAIGN_ONLY_POST.test(block);
}
...
// 2026-09-15: 商品を1つも名指ししない**セール・キャンペーンの告知**（実測 #254779「9月15日10時より
// 楽天トラベル 九州ふっこう応援割が開催 熊本・鹿児島最大60％オフ」がカード名「楽天トラベル」で
// トップに出ていた。他に「駿河屋アーリー・オータムセールが開催」「楽天スーパーセール タイムセールで
// 良さそうなもの選んでみた その1」＝旅行割引・店のセール・買い物メモ）。
// **生タイトルには当てない**: 「楽天ブックスで販売再開中 Xbox Series X … 本日スーパーセール中です」の
// ように、商品を名指しした上でセールに触れる投稿は正当な商品（実測で 8件）。当てるのは
// **表示に選ばれた節**だけ＝節そのものがキャンペーン名なら商品ではない。
const CAMPAIGN_ONLY_POST =
  /楽天トラベル|ふっこう割|応援割|スーパーセール|タイムセール|お買い物マラソン|買いまわり|倍デー|ワンダフルデー|ブランドデー|三太郎の日|ポイント還元|セールが開催|良さそうなもの選んでみた/;
