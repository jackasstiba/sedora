...
// 先頭の「8/22・23発売｜」型の日付告知ラベル（snkrdunk）。**末尾ラベルだけ剥がす実装の穴。**
// 実測 2026-08-22: 生タイトルは「8/22・23発売｜Pharrell Williams × adidas …｜抽選/販売/定価情報」で、
...
export function stripSourceLabel(title: string): string {
  const t = dropUnmatchedBrackets(
    title
      .replace(INVISIBLE, "")
      .replace(SOURCE_LABEL_TAIL, "")
      .replace(SOURCE_LABEL_HEAD, "")
      .replace(SOURCE_LABEL_DATE_HEAD, "")
      .replace(TAG_PIPE, "$1 ")
      .trim()
  );
...
const SEG_TAIL_NOISE = new RegExp(
  "(?:" +
    [
      ...
      "が?多数あり(?:今回も)?",
      // 次の文へ繋ぐ接続副詞が節末に残る型（「〜脹相や伏黒甚爾 再販さらに」）。
      // 全角スペース区切りの投稿で、次節の頭が前節の尻に付く。単体で落とす。
      "(?:さらに|しかも|なお|あわせて|つづいて|続いて)",
...
const SEG_HEAD_NOISE = new RegExp(
  "^[\\s　]*(?:" +
    [
      // 商品タグの**直前に密着した相場の実況**（実測 2026-08-24 #147021:
      // 「…ヤフオクめちゃ高騰していた【特典】東方ぬいぐるみシリーズ …」）。
      // この形は節を割っても切れない——実況と商品名が**同じ節**にあり、しかも節が
      // 商品タグ【】を含むので「商品名の節」と判定されて丸ごと表示に採られる。
      // その結果、**画面に「ヤフオクめちゃ高騰していた」が出る**＝相場・転売の実況を
      // 出さないという掲載方針そのものに反する（[[ハツコレの立ち位置]]）。
      // 安全側の条件: 市場語（商品名には決して現れない＝ALWAYS_COMMENT と同じ前提）を必ず含み、
      // 語の途中で切らないよう**コメントの終止形で終わる**ことを要求し、空白をまたがない。
      "[^、。！!【『「\\s　]{0,8}?(?:高騰|ヤフオク|メルカリ|転売|品薄|即完売)[^、。！!【『「\\s　]{0,6}?(?:していた|してた|している|してる|でした|だった|中の|中|の)",
      "まずは",
      "まさかの",
      "しかも",
...
function isCommentSeg(seg: string): boolean {
  ...
  // 値引き・ポイント告知だけの断片（「最大96％オフ」「ポイント10倍」）。数字を含むので
  // 商品名判定を通ってしまうが、割引語を除いて商品名らしい語が残らないなら販促の実況。
  // 実測: セール告知ツイートの「最大96％オフ」が商品名として採用され一覧に出ていた。
  if (/[０-９0-9]\s*[％%]\s*オフ|ポイント\s*[０-９0-9]+\s*倍|割引/.test(core)) {
    const rest = core
      .replace(/(?:最大|まだ|全品|一部)?[\s　]*[０-９0-9]{1,3}\s*[％%]\s*オフ(?:あり|で)?|ポイント\s*[０-９0-9]+\s*倍|割引/g, "")
      .trim();
