 ...
 */
const SALE_COND =
  // 「お1家族様1回」「タムカ会員限定」（実測 2026-09-13: この2語を知らず、条件が商品名に残って
  // 楽天の全語一致が0件＝画像が付かない行が card_chusen で48件）。家族・会員・N回 を足す。
  /現金|税込|税抜|[\d,]+円|購入権|抽選|予約|販売|再販|再入荷|おひとり|お一人|お1人|一人様|家族|会員|\d+\s*回|発売分|次受付|キャンペーン|店舗告知|数量限定|限定販売|個数限定|台数限定|開封|シュリンク|お渡し|引き渡し|引換|当選|応募|店頭|来店|通販|オンライン|送料|希望小売価格|\d+\s*(?:BOX|箱|パック|カートン|個|セット|点|口|名|種)|全\d+種/i;

export function splitSaleConditions(name: string): { base: string; conds: string[] } {
  const conds: string[] = [];
  // 回次の丸数字（「…「30th CELEBRATION」②（2回目・…）」）。NFKC で「2」に化けて商品名の末尾に
  // 裸で残る（実測 2026-09-12: シーガル各店の2枠が「…」2」で本番に出ていた）。**直後の括弧が
  // 「N回目」と言っている時だけ**落とす（回次は条件側が持つ）。セット番号（御三家カードセット②）
  // は商品の identity なので触らない。
  let t = name.replace(/[①-⑳㉑-㉟]+(?=\s*[（(][^（）()]*回目)/g, " ").normalize("NFKC");
  // 括弧は**1段のネスト**まで読む。「（お一人様（一家族）1BOXまで）」を外側の「）」でなく内側の
  // 「）」で閉じると、「1BOXまで）」が名前に残る（実測 2026-09-12: BOOKOFF PLUS 千葉仁戸名店の
  // 2枠が「… まで)」で本番に出ていた）。
  ...
  // 括弧の外に出る金額（「…3種(イーブイex/…)各1,800円税込」）。価格は条件であって商品名ではない
  // （実測 2026-09-13: 末尾の金額が検索語に残り、画像の全語一致が0件）。
  t = t.replace(/各?\s*[\d,]+\s*円\s*(?:\(?税込\)?|\(?税抜\)?)?/g, (m) => {
    conds.push(m.trim());
    return " ";
  });
