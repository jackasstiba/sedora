...
const NON_IDENTITY_TOKEN =
  /^(再販|再販分|再入荷|再入荷分|追加入荷|追加入荷分|入荷分|購入権|購入権チケット|購入権抽選|購入券|購入券抽選|抽選|抽選販売|抽選受付中|受付中|予約|予約販売|予約受付中|応募|数量限定|など|など多数|新品|中古|未開封|送料無料|正規品|即納|新型)$/;
...
export function searchQueryName(title: string): string {
  ...
  // ⓪ 末尾の金額（「…3種(…)各1,800円税込」）。価格は条件であって商品名ではない。括弧より先に
  //   落とす（括弧の後ろに付くので、これが残ると①の「末尾の括弧」に当たらない。実測 2026-09-13）。
  s = s.replace(/\s*各?\s*[\d,]+\s*円\s*(?:\(?税込\)?|\(?税抜\)?)?\s*$/u, "");
  for (let i = 0; i < 3; i++) {
    const m = s.match(/[（(]([^（()）]*)[)）]?\s*$/);
    if (m?.index === undefined || !SALE_CONDITION_RE.test(m[1])) break;
    s = s.slice(0, m.index).trim();
  }
  // ①' まとめ見出しの接尾語を落とす（nyuka_now の「◯◯の抽選予約・先着販売・在庫あり・
  //    再販入荷情報まとめ」型）。販売状況の語が「・」で連なって終わる形だけを末尾から剥がす。
  //    実測 2026-08-21: この接尾語が検索語に残り、商品は楽天に普通に並んでいるのに全語一致が
  //    0件だった（「ポケモンカード インフェルノX の抽選予約・…まとめ」）。
  s = s.replace(/の?(?:(?:抽選予約|先着販売|抽選|先着|在庫あり|再販入荷|入荷|販売|予約|情報|まとめ)[・\s]*)+$/u, " ");
  // ①'' 「/オフィシャルカードスリーブ」のような**同梱アクセサリの併記**を末尾から落とす。
  //    抽選が「パック＋スリーブ」を同時に扱う行で、スリーブの語が本体の出品名に無く全語一致が
  //    壊れる（実測 2026-08-21: OP-17 世界最強の戦士）。画像は本体側の出品から借りる。
  s = s.replace(/[/／]\s*[^/／]*(?:スリーブ|プロテクター|ローダー|バインダー|デッキケース)[^/／]*$/u, " ");
  // ② 括弧に入っていない販売条件の語も落とす。
  return s
    .replace(
      /[（(【\[]?\s*(再販分|再販|再入荷分|再入荷|追加入荷分|追加入荷|購入権抽選|購入権チケット|購入権|購入券抽選|購入券|抽選販売|予約販売|数量限定|1BOX|1box)\s*[)）】\]]?/g,
      " "
    )
    .replace(/\s+/g, " ")
    .trim();
...
const SALE_CONDITION_RE =
  // 家族・会員・N回 は 2026-09-13 追加（「(お1家族様1回・タムカ会員限定)」が残って画像0件）。
  /再販|再入荷|追加入荷|購入権|抽選|予約|お一人様|おひとり様|おひとりさま|お1人様|1人\d|家族|会員|[0-9]+\s*回|発売分|次受付|キャンペーン|店舗告知|[0-9]+\s*(?:BOX|box|個|点|パック)まで|税込|税抜|現金|シュリンク|開封|事前|先着|購入制限|数量限定/;
...
/**
 * 収集元の略号 ↔ 店の正式表記の対応表（**実際の出品タイトルを見て裏取りした組だけ**足す）。
 * query側の語が候補名に無いとき、対応する正式表記が候補名にあれば一致とみなす。
 * 「緩める」のではなく、検証済みの同義だけを通す（推測の同義を足さないこと）。
 * 実測 2026-08-21: ガンプラ再販の SDEX/ACVI、トレカ抽選の「御三家カードセット」が、
 * 楽天の検索1位が正解の商品なのに表記差だけで全滅していた。
 *   - SDEX … 店は「SDガンダム EXスタンダード」と書く（グレード名。正規化で空白が落ちるので
 *             "exスタンダード" の連続一致で判定できる）
 *   - ACVI … 店は「ARMORED CORE VI（6）」と書く
 *   - 御三家カードセット … 店は「カードセット」＋御三家の名前の列挙（フシギダネ・…）で書く。
 *             どの御三家かは**ポケモン名のトークンが別途全語一致で担保**される
 */
// 表は読みやすい表記で書き、**比較はどちらも normalizeName を通した形**で行う
// （正規化は長音「ー」を落とすので、生の表記のまま includes すると絶対に当たらない。実測で踏んだ）。
const TOKEN_EQUIVALENTS_RAW: Record<string, string[]> = {
  SDEX: ["EXスタンダード"],
  ACVI: ["ARMORED CORE"],
  御三家カードセット: ["カードセット"],
};
const TOKEN_EQUIVALENTS: ReadonlyMap<string, string[]> = new Map(
  Object.entries(TOKEN_EQUIVALENTS_RAW).map(([k, v]) => [normalizeName(k), v.map(normalizeName)])
);
...
export function productNameMatches(query: string, candidate: string): boolean {
  ...
  for (const [i, t] of tokens.entries()) {
    ...
    // 略号 ↔ 正式表記の対応表（上の TOKEN_EQUIVALENTS。裏取り済みの組だけ）。
    if (TOKEN_EQUIVALENTS.get(t.n)?.some((alt) => cand.includes(alt))) continue;
