...
async function main() {
  ...
  //      裏取り済みの事実だけを約束する原則（[[UIラベルは裏取り済みのみ約束]]）の日付版。
  {
    ...
    for (const r of shown) {
      ...
      // 収集元の**画面部品の文言**（開閉ボタン等）が日付欄に出る型。実測 2026-08-20:
      // torecasoku の「未定」セクション見出し「▼ 未定 商品を表示 ▼」を日付の文言として
      // 保存し、61件の日付欄にそのまま出ていた（観点Aの実物突合で発覚）。
      // 「未定」の語を含むので語彙では拾えない。矢印記号と操作動詞で拾う。
      if (/[▼▲◀▶►◄]|を表示|をタップ|をクリック/.test(label))
        bad.push(`[${r.source} #${r.id}] 日付欄に画面部品の文言 «${label}» ${cleanListTitle(r.source, r.title)}`);
