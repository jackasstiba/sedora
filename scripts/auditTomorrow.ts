...
const CALENDAR_EXPECTED = new Set([
  ...
  "watchlist_zero",
  // 「巡回できているのに表示0件のソース」の検査。この道具は**データを凍結したまま暦だけ**
  // 進めるので、在籍が薄いソース（実測 2026-08-20: sofvi＝DB4件）は手持ちの行が過去日になる
  // だけで必ず0件に落ちる＝暦で鳴る。実運用では巡回が先に走って行が入れ替わるので、
  // 本物の「取得は成功するが中身が全部スコープ外」は通常の `npm run audit` のラチェット
  // （WARN・増えたらERROR）が捕まえる。watchlist_zero と同じ理由。
  ...
  // 日付ロールオーバー（cron）が止まっていないかを見る検査。この道具は**暦だけ**進めるので、
  // 未来では「最後に動いてから24時間以上経った」状態に必ずなる（未来の cron はまだ動いていない）。
  // 本物の「cronが死んだ」は、その日が来たときの通常の `npm run audit` が捕まえる。
